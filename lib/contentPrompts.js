...
// Claude's own character counting isn't perfectly precise, so this is a
// safety net: if the model overshoots the requested budget, trim the main
// text (never the source credit) down to fit, cutting at a word boundary
// rather than mid-word.
export function enforceLimit(mainText, suffix, limit) {
  const combined = mainText + suffix;
  if (combined.length <= limit) return combined;

  const allowedMainLength = Math.max(0, limit - suffix.length);
  let trimmed = mainText.slice(0, allowedMainLength);
